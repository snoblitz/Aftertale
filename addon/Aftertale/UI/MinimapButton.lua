...
local function buildButton()
  ...
  btn:SetScript("OnEnter", function(self)
    ...
    GameTooltip:AddLine("This session: " .. NS.session.events .. " events, "
      ...
      0.6, 0.8, 0.6)
    GameTooltip:AddLine(" ")
    GameTooltip:AddLine("|cFFFFD700Left-click|r  Open Aftertale", 0.9, 0.9, 0.9)
...
if StaticPopupDialogs then
  ...
  btn:SetScript("OnClick", function(self, button)
    if button == "LeftButton" then
      -- The addon's front door: a quiet status / control popover with the
      -- live player model and the two real verbs (hold, pause). NOT the
      -- chronicle book -- that's hidden behind config.enableInGameBook now
      -- and isn't a reader for the cohort.
      if NS.OpenPopover then
        NS.OpenPopover()
      elseif NS.OpenBook and NS.GetConfig().enableInGameBook then
        NS.OpenBook()
      end
    elseif button == "RightButton" then
      -- Right-click opens Settings. The minimap button no longer surfaces
      -- the web URL or /at sync -- the addon is silent about the website
      -- per Blizzard addon policy, and the popover is the main verb.
